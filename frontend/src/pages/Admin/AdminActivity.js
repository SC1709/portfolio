...
function AdminActivity() {
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (selectedItemForEdit) {
        response = await axios.post(`${apiUrl}/api/portfolio/update-activity`, {
          ...values,
          _id: selectedItemForEdit._id,
        });
      } else {
        response = await axios.post(`${apiUrl}/api/portfolio/add-activity`, values);
      }
  ...
  const onDelete = async (item) => {
    try {
      dispatch(ShowLoading());
      const response = await axios.post(`${apiUrl}/api/portfolio/delete-activity`, {
        _id: item._id,
      });
      dispatch(HideLoading());
      if (response.data.success) {
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-5 mt-5 sm:grid-cols-1">
        {activities.map((activity) => (
          <div className="shadow border p-5 border-gray-400 flex flex-col gap-2">
            ...
            <hr />
            <h1>Place : {activity.place}</h1>
            <h1>Year : {activity.year}</h1>
      ...
      {(type === "add" || selectedItemForEdit) && (
        ...
        >
          ...
          >
            <Form.Item name="year" label="Period">
